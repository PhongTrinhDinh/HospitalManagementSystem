...
public:
    ...
    std::vector<Patient*> searchByName(const std::string& name);
    std::vector<Patient*> searchByAge(int age);

    // Helper method để hiển thị kết quả tìm kiếm
    void displaySearchResults(const std::vector<Patient*>& results, 
