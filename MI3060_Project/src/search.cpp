...
Patient* PatientSearch::searchByID(const std::string& id) const {
    auto results = idTable.search(id);
    if (!results.empty()) {
...
std::vector<Patient*> PatientSearch::searchByName(const std::string& name) const {
    return nameTable.search(name);
}

Patient* PatientSearch::searchByPhone(const std::string& phone) const {
    auto results = phoneTable.search(phone);
    if (!results.empty()) {
        return results.front();
    }
    return nullptr;
...
std::vector<Patient*> PatientSearch::searchByAge(int age) const {
    if(age < 0 || age > 150) {
